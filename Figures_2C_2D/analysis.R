...
for (i in c(1:length(file_names))) {
  ...
  write.FCS(sub_data_list[[i]], paste(sub(".fcs","",file_names[[i]]),"_subset",".fcs", sep = ""))
...
md$patient_id <- read.table(text = md$file_name, sep = "-", as.is = TRUE)$V1
md$condition <- c("TP1")
md[grep("TP2",md$file_name),]$condition <- c("TP2")
md[grep("TP3",md$file_name),]$condition <- c("TP3")
md[grep("TP4",md$file_name),]$condition <- c("TP4")
md[grep("TP5",md$file_name),]$condition <- c("TP5")
md[grep("TP6",md$file_name),]$condition <- c("TP6")
...
#panel[grep("CD28",panel$antigen),]$marker_class <- c("state")
...
sce <- runDR(sce, "DiffusionMap", features = "type")
sce <- runDR(sce, "UMAP", features = "type")

library(ggplot2)
plotDR(sce, "TSNE") + geom_point(size=1)
plotDR(sce, "DiffusionMap") + geom_point(size=1)
...
cdx <- rownames(sce)[c(1,3,4,6:10,12:14)]
plotDR(sce, scale = T,color_by = cdx, ncol = 3, a_pal = rev(hcl.colors(10, "Spectral"))) + geom_point(size=0.5) 
plotDR(sce, "DiffusionMap",scale = T,color_by = cdx, ncol = 3, a_pal = rev(hcl.colors(10, "Spectral"))) + geom_point(size=0.5) 
